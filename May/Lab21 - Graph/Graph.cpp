...
#include "../Lab20 - Binary Tree/colors.h"
#include <math.h>
...
void Graph::draw(vector<vector<int>> matrix, vector<string> names) {
	...
	for (int i = 0; i < m_count - 1; i++) {
		for (int j = i + 1; j < count; j++) {
			if (i == j) continue;
			if (matrix[i][j] > 0)
				drawLine(matrix[i][j], m_coord[j].x, m_coord[j].y, m_coord[i].x, m_coord[i].y, -1);
			if (matrix[j][i] > 0)
				drawLine(matrix[j][i], m_coord[i].x, m_coord[i].y, m_coord[j].x, m_coord[j].y, 1);
		}
	}
...
void Graph::drawLine(int w, int x1, int y1, int x2, int y2, int direction) {
