...
int main()
{
	...
	cout << "—генерированные информационные пол€ в списке:" << endl;
	printList(list);

	addZero(list);
	cout << "»нформационные пол€ после добавлени€ нулевых элементов после полей с отрицательными значени€ми:" << endl;
	printList(list);
	removeList(list);
	return 0;
}